...
# Extended material
    ...
    # Terminal Colors
    if termscheme is not None:
        with open(termscheme, 'r') as f:
        ...
        for color, val in term_source_colors.items():
            ...
            if blend_bg_fg and color == "term0":
                ...
                harmonized = harmonize(hex_to_argb(val), primary_color_argb, harmonize_threshold, harmony)
                harmonized = boost_chroma_tone(harmonized, 1, 1 + (term_fg_boost * (1 if darkmode else -1)))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--print', action='store_true', default=False, help='print all created colors')
    args = parser.parse_args()

    generate_color_material(**vars(args))
