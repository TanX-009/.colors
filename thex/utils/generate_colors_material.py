...
def generate_color_material(
    ...
    argb = 0xFFFFFFFF
    hct = Hct.from_int(argb)
    hsize = wsize = wsize_new = hsize_new = 0
        ...
        for color, val in term_source_colors.items():
            ...
            if blend_bg_fg and color == "term0":
                ...
                harmonized = harmonize(
                    hex_to_argb(val), primary_color_argb, harmonize_threshold, harmony
                )
                harmonized = boost_chroma_tone(
                    harmonized, 1, 1 + (term_fg_boost * (1 if darkmode else -1))
                )
            term_colors[color] = argb_to_hex(harmonized)

    material_colors.update(term_colors)

    if not debug and printit:
        ...
        print(
            f"Accent color: {display_color(rgba_from_argb(argb))} {argb_to_hex(argb)}"
        )
